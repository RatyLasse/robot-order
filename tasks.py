...
def order_robots_from_RobotSpareBin():
    ...
    for order in orders:
        ...
        embed_screenshot_to_receipt('preview.png', "output/receipts/" + order_number + "_receipt.pdf")
    archive_receipts()
    remove_receipts_directory()
...
def submit_order():
    ...
    for _ in range(5):  # Retry up to 5 times
        try:
            page.click("#order")
            page.wait_for_selector("#order-completion", timeout=2000) # Tiny wait stage just for sake of testing. TODO: Wait for error message at the same time
...
def remove_receipts_directory():
    """Removes the receipts directory as the receipts are already contained in a zip file"""
    lib = FileSystem()
